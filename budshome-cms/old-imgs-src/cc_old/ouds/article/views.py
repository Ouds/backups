...
from ouds.settings import HOST_URL, ICON_SIZE, IMAGE_SIZE
from ouds.utils.comms import _md5_key
from ouds.article.models import Catalog, Tag, Topic, Entry, Comment

################################################

def module(request, module, template_name = 'article/main.ouds'):
...
def catalog(request, module, catalog, template_name = 'article/main.ouds'):
...
def tag(request, module, catalog, tag, template_name = 'article/main.ouds'):
    
    user = request.user

    tag = Tag.objects.get(catalog__module__exact = module, name__exact = tag)
    tag.read_count += 1
    tag.save()
...
from ouds.utils.consts import IMG_TYPE
