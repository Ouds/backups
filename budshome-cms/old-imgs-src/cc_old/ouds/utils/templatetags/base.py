...
def tags(catalog):
    ...
    if catalog:
        tags = tags.filter(catalog__exact = catalog)[:300]
    else:
        tags = tags[:300]
...
def topics(module, catalog, tag, count = 12):
    '''模块下最新文章列表'''

    latest_topics = Topic.published.filter(catalog__module__exact = module)
    if not module:
        latest_topics = Topic.published.order_by('-comment_count')[:count]
    elif module and not catalog:
        latest_topics = latest_topics[:count]
    elif catalog and not tag:
        latest_topics = latest_topics.filter(catalog__name__exact = catalog)[:count]
    elif tag:
