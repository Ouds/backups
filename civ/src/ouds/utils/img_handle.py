...
MARK_IMG = 'ddq.gif'
POSITION = ('LEFT_TOP', 'RIGHT_TOP', 'CENTER', 'LEFT_BOTTOM', 'RIGHT_BOTTOM')
PADDING = 10
...
def watermark(img, mark_img, position = POSITION[4], opacity = 1):
    ...
    elif position == POSITION[0]: # left top
        position = (PADDING,PADDING)
        layer.paste(mark, position)
    elif position == POSITION[1]: # right bottom
        position = (img.size[0] - mark.size[0]-PADDING, PADDING)
        layer.paste(mark, position)
    elif position == POSITION[2]: # center
        position = ((img.size[0] - mark.size[0])/2,(img.size[1] - mark.size[1])/2)
        layer.paste(mark, position)
    elif position == POSITION[3]: # left bottom
        position = (PADDING,img.size[1] - mark.size[1]-PADDING,)
        layer.paste(mark, position)
    else: # right bottom (default)
...
def test():
    watermark('3.jpg',MARK_IMG,POSITION[0],opacity=0.7).save("watermarked_lt.jpg",quality=90)
    watermark('3.jpg',MARK_IMG,POSITION[1],opacity=0.7).save("watermarked_rt.jpg",quality=90)
    watermark('3.jpg',MARK_IMG,POSITION[2],opacity=0.7).save("watermarked_center.jpg",quality=90)
    watermark('3.jpg',MARK_IMG,POSITION[3],opacity=0.7).save("watermarked_lb.jpg",quality=90)
    watermark('3.jpg',MARK_IMG,POSITION[4],opacity=0.7).save("watermarked_rb.jpg",quality=90)
