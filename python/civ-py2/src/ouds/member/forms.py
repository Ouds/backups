...
from ouds.utils.consts import SEX_TYPE, USER_AREA, MEMBER_TYPE
...
class RegistrationForm(forms.Form):
    ...
    password = forms.CharField(label = _(u'游戏密钥'), widget = forms.PasswordInput, help_text = u' * 五个字符或以上')
    ...
    def clean_username(self):
        if len(self.cleaned_data['username']) < 3:
            raise forms.ValidationError(_(u'用户帐号最少需要3个字符。'))
        try:
...
class RegistrationFormTermsOfService(RegistrationForm):
    ...
    """
    tos = forms.BooleanField(label=_(u'I have read and agree to the Terms of Service'), widget=forms.CheckboxInput())
