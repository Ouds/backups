...
from ouds.utils.consts import MARGIN_ROWS, MARGIN_COLUMNS, MIN_LONGITUDE, MAX_LONGITUDE, MIN_LATITUDE, MAX_LATITUDE
...
def map(request, coordinate_form = CoordinateForm, template_name = 'map/map.ouds'):
    ...
    if request.method == 'POST':
        ...
        if request.GET:
            get = request.GET
            longitude = int(get['longitude']) + int(get['lm']) * MARGIN_ROWS
            if longitude > MAX_LONGITUDE:
                longitude = MAX_LONGITUDE
            if longitude < MIN_LONGITUDE:
                longitude = MIN_LONGITUDE
            latitude = int(get['latitude']) + int(get['tm']) * MARGIN_COLUMNS
    ...
    left = longitude - MARGIN_ROWS
    if left < MIN_LONGITUDE:
        left = MIN_LONGITUDE
    right = longitude + MARGIN_ROWS
    if right > MAX_LONGITUDE:
        right = MAX_LONGITUDE
    top = latitude + MARGIN_COLUMNS
    if top > MAX_LATITUDE:
        top = MAX_LATITUDE
    bottom = latitude - MARGIN_COLUMNS
